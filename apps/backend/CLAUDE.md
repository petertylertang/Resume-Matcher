...
- **Provider abstraction:** LiteLLM. Providers: `openai`, `openai_compatible` (llama.cpp/vLLM/LM Studio), `opencode` (OpenCode Go), `anthropic`, `openrouter`, `gemini`, `deepseek`, `groq`, `ollama`. `get_model_name()` maps provider→LiteLLM prefix; `_normalize_api_base()` fixes `/v1/v1` duplication per provider.
- **`_PROVIDER_DEFAULT_API_BASE`:** a provider with exactly one correct endpoint gets it filled in when the user stored no `api_base`; an explicit value always wins. Only `opencode` qualifies today, and the reason is a trap: one key serves both `https://opencode.ai/zen/go/v1` (the flat-rate Go subscription) and `/zen/v1` (Zen, pay-per-use), and on a Go plan the Zen path 401s for paid models while `*-free` models still complete — which reads as a model-entitlement wall, not a wrong URL. `openai_compatible` deliberately does NOT get a default: it has no single right answer, so a missing base must keep failing loudly.
